...
trait SqlFormatterListTrait {
  ...
  private function isCompactDataTypeKeyword(array $tokens, int $i): bool {
    if (!in_array($tokens[$i]['upper'], \MADB\Query\SqlLexicon::DATA_TYPES, true)) {
      return false;
    }
    $next = $i + 1;
  ...
  private function isCompactParenthesis(array $tokens, int $open): bool {
    ...
    if ($previous['type'] === self::TYPE_KEYWORD && in_array($previous['upper'], \MADB\Query\SqlLexicon::DATA_TYPES, true)) {
      return true;
    }
