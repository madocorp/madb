  ...
  private function tokenize(string $sql): array {
    ...
    while ($offset < $length) {
      ...
      if (preg_match('/^[A-Za-z_][A-Za-z0-9_$]*/', $remaining, $match)) {
        ...
        $type = in_array($upper, \MADB\Query\SqlLexicon::KEYWORDS, true) || in_array($upper, \MADB\Query\SqlLexicon::DATA_TYPES, true) || in_array($upper, \MADB\Query\SqlLexicon::CONSTANTS, true)
          ? self::TYPE_KEYWORD
          : self::TYPE_WORD;
